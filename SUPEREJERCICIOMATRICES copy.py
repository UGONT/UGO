...
def arreglo_listas(lista1,lista2):
    arreglo_lista2 = np.array(lista2)
    arreglo_lista1 = np.array(lista1)
    return arreglo_lista1,arreglo_lista2

def matri_pro(arreglo_lista1,NumFaenas,NumDias):
...
def PlanProd(matriz_pro):#matriz plan de produccion
    n = 1
    for fila in matriz_pro:
...
while op != 5:
    try:
        ...
        if op == 1:
            ...
            Enumeracion_dias()
            arreglo_listas(lista1,lista2)
            matri_pro(NumFaenas,NumDias)
